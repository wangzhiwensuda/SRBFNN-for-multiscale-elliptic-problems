...
def train(net,data_iter,eps,device,MaxNiter,SparseNiter,lr,tol1,tol2,Check_iter,lam1,lam2,lam3):
    ...
    for Niter in range(1, MaxNiter + 1):
        ...
        if (Niter % Check_iter == 0):
            ...
            if not os.path.exists('model/exam4/'):
                os.makedirs('model/exam4/')
            #torch.save(net.state_dict(), 'model/exam4/1d_exam4_%.3f.pth' % (eps))
            net = net.cpu()
...
def load_pth(net,eps):
    if os.path.exists('model/exam4/1d_exam4_%.3f.pth' % (eps)):
        ...
        ckpt = torch.load('model/exam4/1d_exam4_%.3f.pth' % (eps))
...
eps=0.01    374             Rel. L2: 0.0003358968623856699 Rel. L_inf: 0.0006156892522448946 Rel. H1: 0.017650683936175016
eps=0.005   605             Rel. L2: 0.000671383024049742 Rel. L_inf: 0.0009059781024788498 Rel. H1: 0.024577677769257
eps=0.002   965            Rel. L2: 0.00022178762040493875 Rel. L_inf: 0.0006465924593384188 Rel. H1: 0.03832141252229796
"""
