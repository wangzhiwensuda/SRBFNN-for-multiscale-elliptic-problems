...
def generate_FDM(eps):
    if os.path.exists('2d_FDM/2d_exam6_FDM_%.3f.npy'% (eps)): 
        ...
        raise ValueError("Run: python 2d_exam6_FDM.py, to generate the FDM solution")
...
def train(net,data_iter,batch_size_bd,eps,device,MaxNiter,SparseNiter,lr,tol1,tol2,Check_iter,lam1,lam2,lam3):
    ...
    for Niter in range(1, MaxNiter + 1):
        ...
        if (Niter % Check_iter == 0):
            ...
            if not os.path.exists('model/exam6/'):
                os.makedirs('model/exam6/')
            #torch.save(net.state_dict(), 'model/exam6/2d_exam6_%.3f.pth' % (eps))
...
def load_pth(net,eps):
    if os.path.exists('model/exam6/2d_exam6_%.3f.pth' % (eps)):
        print('load the trained model')
        ckpt = torch.load('model/exam6/2d_exam6_%.3f.pth' % (eps))
...
"""         N
eps=0.5      48         Rel. L2: 0.0013199815116414894 Rel. L_inf: 0.01762766339874231 Rel. H1: 0.012758795113875343  
eps=0.2      152        Rel. L2: 0.0021167296001972052 Rel. L_inf: 0.02700569109430595 Rel. H1: 0.01650158237661742
eps=0.1     538         Rel. L2: 0.003049107692384658 Rel. L_inf: 0.02435902982607704 Rel. H1: 0.03496956242144384
eps=0.05    2275        Rel. L2: 0.0031500694052206003 Rel. L_inf: 0.01076415721686232 Rel. H1: 0.0777352617928139
eps=0.02    6362        Rel. L2: 0.0033949510347046573 Rel. L_inf: 0.009815129000034693 Rel. H1: 0.09972138489438817
eps=0.01    17285       Rel. L2: 0.0024814231029485377 Rel. L_inf: 0.004999745127955685 Rel. H1: 0.11250656983473477
"""
