...
def dataloader(h, batch_size, eps1,eps2,eps3,eps4,eps5):
    c1 = 2 * math.pi
    x0 = torch.linspace(0, 1, int(1 / h + 1))
    x,y = torch.meshgrid(x0,x0)
    x,y = x.flatten(),y.flatten()
    X = torch.cat((x.view(-1, 1), y.view(-1, 1)), dim=1)
    a = ((1.1+torch.sin(c1*x/eps1))/(1.1+torch.sin(c1*y/eps1))+
	     (1.1+torch.sin(c1*y/eps2))/(1.1+torch.cos(c1*x/eps2))+
		(1.1 + torch.cos(c1* x / eps3)) / (1.1 + torch.sin(c1*y/eps3)) +
		(1.1 + torch.sin(c1* y/eps4)) / (1.1 + torch.cos(c1*x/eps4)) +
		(1.1 + torch.cos(c1* x / eps5)) / (1.1 + torch.sin(c1* y / eps5))+
		torch.sin(4*x**2*y**2)+1)/6
...
def generate_FDM():
    if os.path.exists('2d_FDM/2d_exam7_FDM.npy'): 
        u_FDM = np.load('2d_FDM/2d_exam7_FDM.npy')[::2,::2]
        ux_FDM = np.load('2d_FDM/2d_exam7_FDM_x.npy')[::2,::2][1:-1,1:-1]
        uy_FDM = np.load('2d_FDM/2d_exam7_FDM_y.npy')[::2,::2][1:-1,1:-1]
    else:
        raise ValueError("Run: python 2d_exam7_FDM.py, to generate the FDM solution")
...
#The RBFNN's derivatives about x:
...
#The RBFNN's derivatives about y:
...
def get_bound_loss(net,device, batch_size_bd=512):  
    ...
    bound_loss = ((get_u(X_1,net.center,net.hight,net.width) - 0) ** 2).mean() + ((get_u(X_2,net.center, net.hight, net.width) - 0) ** 2).mean() + \
                 ((get_u(X_3,net.center,net.hight,net.width) - 0) ** 2).mean() + ((get_u(X_4,net.center, net.hight, net.width) - 0) ** 2).mean()
    return bound_loss



#calculate the three relative errors
def get_err(net,eps1,eps2,eps3,eps4,eps5,device,dx = 0.001):
    ...
    u_FDM, ux_FDM,uy_FDM = generate_FDM()
...
def train(net,data_iter,batch_size_bd,eps1,eps2,eps3,eps4,eps5,device,MaxNiter,SparseNiter,lr,tol1,tol2,Check_iter,lam1,lam2,lam3):
    ...
    for Niter in range(1, MaxNiter + 1):
        ...
        print('t_all=%.3f,t_Niter=%.3f,thres=%.4f,Niter:%d,l_P:%f,l_Q:%f,l_f:%f,l_bd:%f,l_all:%f' 
            % (t_all,t2-t1, thres, Niter, l_P_sum,l_Q_sum, l_f_sum,l_bd_sum, l_sums))
        ...
        if (Niter % Check_iter == 0):
            ...
            if not os.path.exists('model/exam7/'):
                os.makedirs('model/exam7/')
            #torch.save(net.state_dict(), 'model/exam7/2d_exam7.pth')
            ...
            L2,L_inf,H1 = get_err(net,eps1,eps2,eps3,eps4,eps5,device)
            print('Rel. L2:', L2,'Rel. L_inf:', L_inf,'Rel. H1:', H1)

#load the trained model
def load_pth(net):
    if os.path.exists('model/exam7/2d_exam7.pth'):
        print('load the trained model')
        ckpt = torch.load('model/exam7/2d_exam7.pth')
...
def show_pointwise_error(net,device, dx=0.001):
    x0 = torch.linspace(0, 1, int(1 / dx + 1))
    x, y = torch.meshgrid(x0, x0)
    x, y = x.flatten(), y.flatten()
    X = torch.cat((x.view(-1, 1), y.view(-1, 1)), dim=1)  # [[0,0],[0,0.001],...]
...
# show cross-section view of u, ux and uy 
def show_cross_section_view(net,eps1,eps2,eps3,eps4,eps5):
    net = net.cpu()
    c1 = 2*math.pi
    x0 = torch.linspace(0, 1, 1001)
    y0 = 0.5 * torch.ones_like(x0)
    X1 = torch.cat((x0.view(-1, 1), y0.view(-1, 1)), dim=1)  
    X2 = torch.cat((y0.view(-1, 1), x0.view(-1, 1)), dim=1)  
    x, y = X1[:,0],X1[:,1]
    a1 = ((1.1+torch.sin(c1*x/eps1))/(1.1+torch.sin(c1*y/eps1))+
             (1.1+torch.sin(c1*y/eps2))/(1.1+torch.cos(c1*x/eps2))+
             (1.1 + torch.cos(c1* x / eps3)) / (1.1 + torch.sin(c1 * y / eps3)) +
             (1.1 + torch.sin(c1 * y/eps4)) / (1.1 + torch.cos(c1 * x/eps4)) +
             (1.1 + torch.cos(c1 * x / eps5)) / (1.1 + torch.sin(c1* y / eps5))+
            torch.sin(4*x**2*y**2)+1)/6
    x, y = X2[:, 0], X2[:, 1]
    a2 = ((1.1+torch.sin(c1*x/eps1))/(1.1+torch.sin(c1*y/eps1))+
             (1.1+torch.sin(c1*y/eps2))/(1.1+torch.cos(c1*x/eps2))+
             (1.1 + torch.cos(c1* x / eps3)) / (1.1 + torch.sin(c1* y / eps3)) +
             (1.1 + torch.sin(c1 * y/eps4)) / (1.1 + torch.cos(c1 * x/eps4)) +
             (1.1 + torch.cos(c1 * x / eps5)) / (1.1 + torch.sin(c1* y / eps5))+
            torch.sin(4*x**2*y**2)+1)/6
    with torch.no_grad():
        u1 = get_u(X1, net.center, net.hight, net.width)
        u2 = get_u(X2, net.center, net.hight, net.width)
        ux = get_u(X1, net.center2, net.hight2, net.width2)/a1 
        uy = get_u(X2, net.center3, net.hight3, net.width3)/a2
    u_FDM, ux_FDM,uy_FDM = generate_FDM()
    '--------------------------------'  
    plt.grid(linestyle="--")
    plt.plot(x0, u1,'-*',markevery=50, lw=1.5)
    plt.plot(x0, u_FDM[:, 500],'--',markevery=50, lw=1.5)
    plt.legend(['SRBFNN', 'FDM'],loc="upper left")
    plt.ylabel('u', fontsize=16)
    plt.xlabel('x', fontsize=16)
    plt.show()
    '--------------------------------'
    plt.grid(linestyle="--")
    plt.plot(x0, u2,'-*',markevery=50, lw=1.5)
    plt.plot(x0, u_FDM[500, :],'--',markevery=50, lw=1.5)
    plt.legend(['SRBFNN', 'FDM'],loc="upper left")
    plt.ylabel('u', fontsize=16)
    ...
    '--------------------------------'
    plt.grid(linestyle="--")
    plt.plot(x0, ux,'-*',markevery=50, lw=1.5)
    plt.plot(x0[1:-1], ux_FDM[:, 500],'--',markevery=50, lw=1.5)
    plt.legend(['SRBFNN', 'FDM'])
    plt.ylabel(r'$\frac{\partial{u}}{\partial{x}}$', fontsize=16)
    plt.xlabel('x', fontsize=16)
    plt.show()
    '--------------------------------'
    plt.grid(linestyle="--")
    plt.plot(x0, uy,'-*',markevery=50, lw=1.5)
    plt.plot(x0[1:-1], uy_FDM[500, :],'--',markevery=50, lw=1.5)
    plt.legend(['SRBFNN', 'FDM'])
    plt.ylabel(r'$\frac{\partial{u}}{\partial{y}}$', fontsize=16)
    plt.xlabel('y', fontsize=16)
    plt.show()
...
def main(args):
    ...
    lr = args.lr
    eps1,eps2,eps3,eps4,eps5 = args.eps1,args.eps2,args.eps3,args.eps4,args.eps5
    h = args.h
    lam1,lam2,lam3 = args.lam1,args.lam2,args.lam3
    tol1,tol2 = args.tol1,args.tol2
    Check_iter = args.Check_iter
    data_iter = dataloader(h, batch_size, eps1,eps2,eps3,eps4,eps5)
    net = SRBF2d(N,eps5)
    if args.pretrained:
        net = load_pth(net)
    train(net,data_iter,batch_size_bd,eps1,eps2,eps3,eps4,eps5,device,MaxNiter,SparseNiter,lr,tol1,tol2,Check_iter,lam1,lam2,lam3)
    print('The number of RBFs in final solution: ',net.hight.shape[0])
    L2,L_inf,H1 = get_err(net,eps1,eps2,eps3,eps4,eps5,device)
    print('Rel. L2:', L2,'Rel. L_inf:', L_inf,'Rel. H1:', H1)
    show_pointwise_error(net,device)
    show_cross_section_view(net,eps1,eps2,eps3,eps4,eps5)
   

if __name__ == "__main__":
    parser = argparse.ArgumentParser() 
    parser.add_argument('--eps1', type=float, default=1/5) 
    parser.add_argument('--eps2', type=float, default=1/13)
    parser.add_argument('--eps3', type=float, default=1/17)
    parser.add_argument('--eps4', type=float, default=1/31)
    parser.add_argument('--eps5', type=float, default=1/65)
    ...
    parser.add_argument('--lam1', type=float, default=0.02)
...
"""
N=13213,     Rel. L2: 0.004458838214118196 Rel. L_inf: 0.00924597794828177 Rel. H1: 0.1551113714394088
"""
